...
static char *s_base_url = NULL;

/* ---- URL resolution ---- */

/**
 * Simple URL resolution: handle absolute and relative URLs.
 * Returns a newly allocated string (PSRAM).
 */
static char *resolve_url(const char *href, const char *base)
{
    ...
    /* Find the last '/' in base after the host */
    ...
    if (href[0] == '/' && href[1] == '/') {
        ...
        if (proto_end) {
            int proto_len = proto_end - base + 1;  /* include "://" */
            int href_len = strlen(href);
            char *result = (char *)heap_caps_malloc(proto_len + href_len + 1,
                                MALLOC_CAP_SPIRAM | MALLOC_CAP_8BIT);
            if (result) {
                memcpy(result, base, proto_len);
                strcpy(result + proto_len, href);
            }
            return result;
        }
    }
    ...
    if (href[0] == '/') {
        ...
        if (proto_end) {
            ...
            int href_len = strlen(href);
            char *result = (char *)heap_caps_malloc(host_len + href_len + 1,
                                MALLOC_CAP_SPIRAM | MALLOC_CAP_8BIT);
            if (result) {
    ...
    /* Handle relative URLs (same directory) */
    int base_dir_len = last_slash - base + 1;
    int href_len = strlen(href);
    char *result = (char *)heap_caps_malloc(base_dir_len + href_len + 1,
                        MALLOC_CAP_SPIRAM | MALLOC_CAP_8BIT);
    if (result) {
        memcpy(result, base, base_dir_len);
        strcpy(result + base_dir_len, href);
    }
    return result;
}
...
static void on_link_click(const char *href)
{
    ...
    if (resolved) {
        browser_navigate(resolved);
        heap_caps_free(resolved);
    }
}
...
static void create_ui(void)
{
    ...
    lv_obj_set_style_bg_color(s_url_bar, lv_color_hex(0x2D2D2D), 0);
...
{
    if (s_url_label) {
        lv_label_set_text(s_url_label, url ? url : "");
    }
}
...
{
    ESP_LOGI(TAG, "Initializing browser...");

    /* Initialize LVGL */
    lv_init();

    /* Log PSRAM availability for diagnostics */
    ESP_LOGI(TAG, "Free PSRAM: %d bytes", (int)heap_caps_get_free_size(MALLOC_CAP_SPIRAM));
    ESP_LOGI(TAG, "Free internal: %d bytes", (int)heap_caps_get_free_size(MALLOC_CAP_8BIT));
    ...
    if (ret != ESP_OK) {
        set_status("Wi-Fi connection failed!");
    ...
    ESP_LOGI(TAG, "Browser initialized");
    return ret;
}
...
esp_err_t browser_navigate(const char *url)
{
    ...
    ESP_LOGI(TAG, "Fetched %d bytes", (int)html_len);
    set_status("Parsing...");

    /* 2. Parse HTML */
    dom_node_t *dom = html_parse(html);
    if (!dom) {
        ESP_LOGE(TAG, "HTML parse failed");
        set_status("Parse failed!");
        show_loading(false);
        heap_caps_free(html);
        return ESP_FAIL;
    }

    /* We can free the raw HTML now - DOM has its own copies */
    heap_caps_free(html);

    set_status("Laying out...");

    /* 3. Compute layout */
    int content_width = display_get_h_res() - 8;  /* minus padding */
    layout_box_t *layout = layout_compute(dom, content_width);
    if (!layout) {
        ESP_LOGE(TAG, "Layout computation failed");
        set_status("Layout failed!");
        show_loading(false);
        dom_tree_free(dom);
        return ESP_FAIL;
    }
    ...
    char status_buf[64];
    snprintf(status_buf, sizeof(status_buf), "OK - %dpx", total_h);
    set_status(status_buf);

    /* 6. Cleanup (layout boxes have their own text copies) */
    layout_free(layout);
    dom_tree_free(dom);

    ESP_LOGI(TAG, "Page rendered successfully (%dpx tall)", total_h);
    return ESP_OK;
}
