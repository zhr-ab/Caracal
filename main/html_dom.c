...
static dom_node_t *build_dom(const GumboNode *gnode)
{
    ...
    case GUMBO_NODE_ELEMENT: {
        ...
        /* If normalized tag is empty (unknown tag), try original */
        ...
        /* Auto-detect display:flex / display:grid for common tags */
        if (!node->style.has_style) {
            /* Map semantic tags to appropriate display modes */
            /* nav, header, footer, section → flex row by default */
